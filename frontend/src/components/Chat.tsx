import type { SockeType, user, messages as msg, users } from "../types/types";
...
const Chat: React.FC<Props> = ({ socket, user, messages, setMessages }) => {
  const m = messages.filter(
    (message) =>
      (message.to === socket.id && message.from === user.userId) ||
      (message.to === user.userId && message.from === socket.id)
  );
